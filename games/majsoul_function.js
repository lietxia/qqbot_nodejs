/**
 * @模块 majsoul_function.js
 * @描述 雀魂角色管理功能与麻将牌理/向听/切牌练习功能入口
 */
const fs = require('fs');
const http = require('http');

/**
 * 雀魂角色管理功能（添加/删除/查看/今天玩谁）
 * @param {string} order_string - 指令内容
 * @returns {string} 回复内容
 */
function majsoul_qh_function(order_string) {
	...
	if (order_string.slice(0, 4) === "添加人物") {
		if (majsoul_character_dict[id].indexOf(order_string.slice(4)) >= 0)
			return order_string.slice(4) + "已经在选择池里了~";
		if (雀魂人物.indexOf(order_string.slice(4)) >= 0) {
			majsoul_character_dict[id].push(order_string.slice(4));
			fs.writeFileSync('./games/majsoul_character.js', "majsoul_character_dict=" + JSON.stringify(majsoul_character_dict));
			return order_string.slice(4) + "已进入可使用名单~";
		}
		else
			return "没有这个角色……(不要打任何空格)";
	}
	if (order_string.slice(0, 4) === "删除人物") {
		if (order_string.slice(4) === "一姬")
			return "一姬不想离开你，喵~";
		if (majsoul_character_dict[id].indexOf(order_string.slice(4)) >= 0) {
			majsoul_character_dict[id].splice(majsoul_character_dict[id].indexOf(order_string.slice(4)), 1);
			fs.writeFileSync('./games/majsoul_character.js', "majsoul_character_dict=" + JSON.stringify(majsoul_character_dict));
			return order_string.slice(4) + "已离开可使用名单~";
		}
		else
			return "没有这个角色……(不要打任何空格)";
	}
	if (order_string.slice(0, 4) === "今天玩谁") {
...
/**
 * 麻将功能入口，处理向听/牌理/切牌练习等指令
 * @returns {string} 回复内容
 */
function majsoul_function() {
	fs.writeFileSync('./core/px_log.js', "px_dict=" + JSON.stringify(px_dict));
	if (message.slice(2, 2 + 2) === "向听" || message.slice(2, 2 + 2) === "xt")
		return ((at_sender && type === "group") ? "\n" : "") + santen_reply(convert_to_card(catch_content(4))) + "(" + (Date.now() - start_time) + "ms)";
	if (message.slice(2, 2 + 2) === "牌理" || message.slice(2, 2 + 2) === "pl")
		return ((at_sender && type === "group") ? "\n" : "") + paili_input(catch_content(4)) + "\n(" + (Date.now() - start_time) + "ms)";
	if (message.slice(2, 2 + 4) === "特殊牌理" || message.slice(2, 2 + 4) === "tspl") {
	...
	if (catch_content(2) === "切牌帮助" || catch_content(2) === "qpbz")
		return ((at_sender && type === "group") ? "\n" : "") + mj_px_file;
	if (catch_content(2).length < 5 && (catch_content(2).slice(0, 2) === "切牌" || catch_content(2).slice(0, 2) === "qp"))
		return ((at_sender && type === "group") ? "\n" : "") + px_turn(catch_content(4));
	if (catch_content(2).length < 5 && (catch_content(2).slice(0, 1) === "杠" || catch_content(2).slice(0, 1) === "k"))
		return ((at_sender && type === "group") ? "\n" : "") + px_kong(catch_content(3));
	if (catch_content(2).length < 5 && (catch_content(2).slice(0, 2) === "立直" || catch_content(2).slice(0, 2) === "lz"))
		return ((at_sender && type === "group") ? "\n" : "") + px_reach(catch_content(4));
	if (catch_content(2) === "摸切" || catch_content(2) === "mq")
		return ((at_sender && type === "group") ? "\n" : "") + px_pass();
	if (catch_content(2) === "继续摸切" || catch_content(2) === "jxmq")
		return ((at_sender && type === "group") ? "\n" : "") + one_more_pass();
	if (catch_content(2).length < 5 && (catch_content(2).slice(0, 2) === "继续" || catch_content(2).slice(0, 2) === "jx"))
...
function special_paili(order_string, pl_start = Date.now()) {
	...
	if (dora_string !== "")
		doras = convert_to_card(dora_string.slice(1));
	let red_dora = 0;
	let red_only = [false, false, false];
	let bei_dora = 0;
	if (bei_string !== "")
		bei_dora = parse_if_number(bei_string.slice(1));
...
/**
 * 从和牌结果字符串中提取点数
 * @param {string} mj_result - 和牌结果字符串
 * @returns {number} 点数
 */
 ...
 * @param {Array} red_dora - 红宝状态
 * @param {Array} doras - dora指示牌
 * @param {number} beis - 北dora数量
 * @param {Array} mountain - 牌山
 * @param {number} resume_times - 剩余模拟次数
 * @returns {number} 期望分值
 */
...
/**
 * 从牌面字符串中提取红宝状态
 * @param {string} card_string - 牌面字符串
 * @returns {Array} 红宝状态数组
 */
...
/**
 * 根据手牌生成剩余牌山
 * @param {string} reduce_cards_str - 手牌字符串
 * @returns {Array} 牌山数组
 */
