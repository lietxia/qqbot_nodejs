/**
 * @模块 湮灭之战.js
 * @描述 湮灭之战游戏核心逻辑，包含正反物质碰撞、宇宙垃圾、攻击防御等机制
 */

/**
 * 湮灭之战命令解析入口函数
 * @param {string} order_string - 用户输入的命令字符串
 * @returns {string} 回复内容
 */
function annihilateWar_function(order_string) {
	...
	if (order_option === "排行榜" && order_name === "" && order_detail === "")
		return (type === "group" ? "\n" : "") + aw排行榜();
	if (order_option === "重启服务器" && order_name === "" && order_detail === "" && masterlist.indexOf(id) >= 0)
		return (type === "group" ? "\n" : "") + annihilateWarInitialize();
	if (order_option === "帮助" && order_name === "" && order_detail === "")
		return (type === "group" ? "\n" : "") + aw_help_string;
	if (order_option === "环境" && order_name === "" && order_detail === "")
		return (type === "group" ? "\n" : "") + aw环境();
	if (order_option === "日志" && order_name === "" && order_detail === "")
		return (type === "group" ? "\n" : "") + aw日志();
	if (order_option === "创建" && order_name !== "" && order_detail !== "")
		return (type === "group" ? "\n" : "") + aw创建(order_detail, order_name);
	if (order_option === "删除" && order_name !== "" && order_detail === "")
		return (type === "group" ? "\n" : "") + aw删除(order_name);
	if (order_option === "查看" && order_name !== "" && order_detail === "")
		return (type === "group" ? "\n" : "") + aw查看(order_name);
	if (order_option === "位置" && order_name !== "" && order_detail === "")
		return (type === "group" ? "\n" : "") + aw位置(order_name);
	if (order_option === "改变密度" && order_name !== "" && order_detail !== "")
		return (type === "group" ? "\n" : "") + aw改变密度(order_name, (function(){try{return JSON.parse(order_detail)}catch(e){return 0}})());
	if (order_option === "提升幸运" && order_name !== "" && order_detail !== "")
		return (type === "group" ? "\n" : "") + aw提升幸运(order_name, (function(){try{return JSON.parse(order_detail)}catch(e){return 0}})());
	if (order_option === "提升攻击力" && order_name !== "" && order_detail !== "")
		return (type === "group" ? "\n" : "") + aw提升攻击力(order_name, (function(){try{return JSON.parse(order_detail)}catch(e){return 0}})());
	if (order_option === "提升防御力" && order_name !== "" && order_detail !== "")
		return (type === "group" ? "\n" : "") + aw提升防御力(order_name, (function(){try{return JSON.parse(order_detail)}catch(e){return 0}})());
	if (order_option === "攻击" && order_name !== "" && order_detail !== "")
		return (type === "group" ? "\n" : "") + aw_attack(order_name, order_detail);
	if (order_option === "距离" && order_name !== "" && order_detail !== "")
		return (type === "group" ? "\n" : "") + aw距离(order_name, order_detail);
	if (order_option === "大灾变" && order_name !== "" && order_detail === "")
		return (type === "group" ? "\n" : "") + aw大灾变(order_name);
	return "命令有问题呢~";
}

function aw_attack(name_1, detail) {
	let name_2 = "";
	let times = 0;
	let i = 0;
	...
	for (; i < detail.length; ++i)
		if (detail[i] < "0" || detail[i] > "9")
			...
			times = 10 * times + (detail[i] - "0");
	return aw攻击(name_1, name_2, times);
}
...
function isPointInSpherome(x, y, z, rs, xs, ys, zs) {
	if (distance3D(x, y, z, xs, ys, zs) > rs) {
		return false
	}
	else {
		return true
...
function aw查看(name) {
	...
	if (annihilateWarMatterDate[name]) {
		...
		str += annihilatedMatterDate[name]["type"] + name + "已经堙灭了\n"
