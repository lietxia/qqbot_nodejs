/**
 * @模块 24dot.js
 * @描述 24点游戏模块，支持生成扑克牌面、求解算式，可自定义目标数和牌数
 */
...
/** @type {string[]} 四则运算符列表 */
operator_list = ["+", "-", "*", "/"];
/** @type {string[]} 逆运算符列表，用于求解时反向运算 */
anti_operator_list = ["-", "+", "/", "*"];

/**
 * 生成24点题目
 * @param {number} [n=24] - 目标数字
 * @param {number} [m=4] - 扑克牌数量
 * @returns {string} 生成的扑克牌面字符串
 */
...
/**
 * 求解24点
 * @param {string} order_string - 用户输入，格式为"目标数,牌1,牌2,..."
 * @returns {string} 求解表达式或"无解~"
 */
...
/**
 * 检查牌组是否有重复
 * @param {string[]} the_cards - 牌数组
 * @returns {boolean} 是否无重复
 */
...
/**
 * 将扑克牌面转换为数字(A=1,J=11,Q=12,K=13)
 * @param {string} the_card - 牌面字符串
 * @returns {number} 对应数字
 */
...
/**
 * 两数四则运算
 * @param {number} number_a - 左操作数
 * @param {number} number_b - 右操作数
...
/**
 * 递归求解数字组是否能通过四则运算得到目标值
 * @param {number[]} numbers - 可用数字数组
 * @param {number} target - 目标值
 * @returns {string|boolean} 求解表达式或false
 */
	...
	for (let i = 1; i < power_set.length; ++i) {
		...
		for (let the_result in power_set_result) {
			if (the_result === "Infinity" || the_result === "0")
				continue;
			try { the_result = JSON.parse(the_result); } catch(e) { return "计算结果格式错误"; }
...
/**
 * 判断一个数是否近似整数
 * @param {number} number - 待判断数值
 * @returns {boolean} 是否近似整数
 */
...
/**
 * 计算一组数字通过四则运算能得到的所有结果(幂集分解法)
 * @param {number[]} numbers - 数字数组
 * @returns {object} 结果映射表，键为结果值，值为对应表达式
 */
	...
	for (let i = 1; i < power_set.length; ++i) {
		...
		for (let composition in results) {
			if (counted_compose[composition])
				continue;
			let the_composition; try { the_composition = JSON.parse(composition); } catch(e) { return "算式格式错误"; }
					...
					for (let right in results[the_remain])
						for (let j = 0; j < operator_list.length; ++j) {
							if (left === Infinity || right === Infinity)
								continue;
							if (operator_list[j] === "+") {
								try { left = JSON.parse(left); } catch(e) { return "计算过程错误"; }
								try { right = JSON.parse(right); } catch(e) { return "计算过程错误"; }
							}
