/**
 * @模块 切牌练习.js
 * @描述 麻将切牌练习模块，提供手牌生成、切牌、立直、自摸等交互式练习功能
 */

/**
 * 将牌面字符串转换为Unicode麻将牌符号
 * @param {string} t - 牌面字符串（如"1m2p3s"）
 * @returns {string} Unicode麻将牌符号字符串
 */
...
/**
 * 解析牌编号为[花色,数字]数组
 * @param {number} card_number - 牌编号(0-33)
 * @returns {number[]} [花色,数字]数组，无效返回[-1,-1]
 */
...
/**
 * 执行开杠操作
 * @param {string} card_name - 杠牌名称（如"1m"）
 * @returns {string} 杠牌结果提示信息
 */
 ...
 * @param {boolean} reached - 是否已立直
 * @returns {boolean} 是否可杠
 */
...
/**
 * 执行切牌操作（正常巡次）
 * @param {string} card_name - 切牌名称（如"1m"）
 * @returns {string} 切牌结果提示信息
 */
...
/**
 * 执行立直操作
 * @param {string} card_name - 立直时切出的牌名称
 * @returns {string} 立直结果提示信息
 */
...
/**
 * 获取宝牌指示牌对应的实际宝牌编号
 * @param {number} card - 宝牌指示牌编号
 * @returns {number} 实际宝牌编号
 */
...
/**
 * 显示当前手牌状态
 * @returns {string} 手牌Unicode显示及宝牌信息
 */
function show_hand() {
	let hand_string = "";
	let dora_string = "";
