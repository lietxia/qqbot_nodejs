/**
 * @模块 和牌牌理.js
 * @描述 麻将和牌牌理计算模块，支持输入解析、面子分解、役种判定、符/翻/点数计算
 */
...
/**
 * @模块 和牌牌理.js
 * @描述 日本麻将和牌牌理计算模块，支持役种判定、符数计算、点数计算
 */
...
/**
 * 解析牌理输入命令，提取手牌、摸牌、副露、条件等信息
 * @param {string} order_string - 用户输入的牌理命令字符串
 * @returns {string} 牌理计算结果文本
 */
/**
 * 解析牌理输入指令，返回牌理计算结果
 * @param {string} order_string - 用户输入的牌理指令字符串
 * @returns {string} 牌理计算结果文本
 */
function paili_input(order_string) {
	...
	if (order_section.length < 2 || !have_hold) {
		...
		if (order_section[0].length === 2)
			...
			hand = convert_to_card(order_section[0].slice(0, order_section[0].length - 2) + order_section[0][order_section[0].length - 1]);
			if (hand_amount(hand) === 0)
				return "输入有问题呢……";
		}
		else {
			hand = convert_to_card(order_section[0].slice(0, order_section[0].length - 2));
			if (hand_amount(hand) === 0)
				return "输入有问题呢……";
		}
		if (order_section[0].slice(order_section[0].length - 2) === "0z")
			return order_string + "的牌理:\n" + "牌型有问题呢……";
		hold = convert_to_hold(order_section[0].slice(order_section[0].length - 2));
		tsumo = true;
		...
		if ((order_section[i][0] < "0" || order_section[i][0] > "9") && order_section[i][0] !== "#" && order_section[i][0] !== "口") {
			...
			switch (order_section[i].length) {
				case 5: if ((order_section[i][0] === "口" || order_section[i][0] === "#") && (order_section[i][3] === "口" || order_section[i][3] === "#") && (order_section[i][1] === order_section[i][2]) && (order_section[i][1] !== "0")) {
					...
					switch (order_section.slice(0, 4)) {
						case "口50口":
						case "口05口":
						case "#50#":
						case "#05#":
				...
				case 4: if (mpsz.indexOf(order_section[i][3]) < 0)
					...
					if ((order_section[i][0] === order_section[i][1]) && (order_section[i][1] === order_section[i][2]) && (order_section[i][0] !== "0"))
						...
						switch (order_section[i].slice(0, 3)) {
							case "123":
							case "213":
							case "312":
							case "132":
 ...
 * @param {number} hold - 摸入牌编号(-1表示无)
 * @param {number} last_i - 上次分解的花色索引
 * @param {number} last_j - 上次分解的数字索引
 * @param {number} last_step - 上次分解的步骤编号
 * @returns {string[][]|string} 面子组合列表，无法分解返回"error"
 */
/**
 * 对数牌进行面子分解（递归）
 * @param {number[][]} hand - 手牌数组
 * @param {number} hold - 和牌编号（-1表示无）
 * @param {number} last_i - 上次分解的花色
 * @param {number} last_j - 上次分解的位置
 * @param {number} last_step - 上次分解的步骤
 * @returns {Array|string} 面子列表，或"error"表示无法分解
 */
 ...
 * 分解字牌为刻子/雀头组合
 * @param {number[][]} hand - 手牌二维数组
 * @param {number} hold - 摸入牌编号
 * @returns {string[]|string} 字牌面子列表，无法分解返回"error"
 */
/**
 * 对字牌进行面子分解
 * @param {number[][]} hand - 手牌数组
 * @param {number} hold - 和牌编号（-1表示无）
 * @returns {Array|string} 字牌面子列表，或"error"表示无法分解
 */
 ...
 * @param {number[][]} hand - 手牌二维数组
 * @param {number} hold - 摸入牌编号
 * @returns {number[][]} 加入摸入牌后的手牌
 */
/**
 * 将和牌加入手牌数组
 * @param {number[][]} hand - 手牌数组
 * @param {number} hold - 和牌编号
 * @returns {number[][]} 加入和牌后的手牌数组
 */
...
/**
 * 判断牌型并返回分解结果（国士/九莲/一般/七对/未和）
 * @param {number[][]} hand - 手牌二维数组
 * @param {number} hold - 摸入牌编号
 * @returns {string|Array} 牌型标识或[面子分解,字牌分解]数组
 */
/**
 * 判断手牌的牌型（一般/国士/九莲/七对/未和）
 * @param {number[][]} hand - 手牌数组
 * @param {number} hold - 和牌编号
 * @returns {string|Array} 牌型标识或面子分解结果
 */
...
/**
 * 将面子字符串标准化为统一格式
 * @param {string|number} mianzi_a - 面子字符串或编号
 * @returns {string} 标准化后的面子字符串
 */
function standard_mianzi(mianzi_a) {
	...
	if (mianzi_a.length === 5)
		return mianzi_a[0] + mianzi_a.slice(3);
	switch (mianzi_a.slice(0, 2)) {
		case "12":
		...
		default: return mianzi_a.slice(0, 2) + mianzi_a.slice(4);
	}
}
 ...
 * @param {number[][]} hand - 手牌数组
 * @param {number} hold - 和牌编号
 * @param {string[]} fulu - 副露列表
 * @param {number[]} ming_kong - 明杠列表
 * @param {number[]} an_kong - 暗杠列表
 * @returns {boolean} 牌型是否合法
 */
 ...
 * @param {number[][]} hand - 手牌二维数组
 * @param {number} hold - 摸入牌编号
 * @param {number} dora - 宝牌数
 * @param {number} red_dora - 红宝牌数
 * @param {number} li_dora - 里宝牌数
 * @param {number} bei_dora - 北宝牌数
 * @param {string[]} fulu - 副露列表
 * @param {number[]} ming_kong - 明杠列表
 * @param {number[]} an_kong - 暗杠列表
 * @param {boolean} tsumo - 是否自摸
 * @param {number} seat - 自风(0-3)
 * @param {number} ground_wind - 场风(0-3)
 * @param {boolean} reached - 是否立直
 * @param {boolean} wreached - 是否两立直
 * @param {boolean} yifa - 是否一发
 * @param {boolean} last_turn - 是否海底/河底
 * @param {boolean} linshang - 是否岭上
 * @param {boolean} qiangkong - 是否枪杠
 * @param {boolean} first_turn - 是否天和/地和
 * @returns {string} 和牌结果详情文本
 */
/**
 * 计算和牌的役种、符数、翻数和点数
 * @param {number[][]} hand - 手牌数组
 ...
 * @param {number[]} an_kong - 暗杠列表
 * @param {boolean} tsumo - 是否自摸
 * @param {number} seat - 自风索引
 * @param {number} ground_wind - 场风索引
 * @param {boolean} reached - 是否立直
 * @param {boolean} wreached - 是否双立直
 * @param {boolean} yifa - 是否一发
 * @param {boolean} last_turn - 是否海底/河底
 * @param {boolean} linshang - 是否岭上开花
 * @param {boolean} qiangkong - 是否枪杠
 * @param {boolean} first_turn - 是否天和/地和
 * @returns {string} 和牌计算结果文本
 */
	...
	for (let i = 0; i < the_paixing[0].length; ++i) {
		...
		for (let j = 0; j < the_paixing[0][i].length; ++j) {
			switch (the_paixing[0][i][j].length) {
				...
				case 4: switch (the_paixing[0][i][j].slice(0, 3)) {
						case "111":
					...
					break;
				case 6: switch (the_paixing[0][i][j].slice(0, 2)) {
						case "11":
		...
		for (let j = 0; j < the_paixing[0][i].length; ++j) {
			...
			if (sanse_list[the_mianzi.slice(0, 3)]) {
				if (sanse_list[the_mianzi.slice(0, 3)].indexOf(the_mianzi[3]) < 0)
					sanse_list[the_mianzi.slice(0, 3)].push(the_mianzi[3]);
			}
			else
				sanse_list[the_mianzi.slice(0, 3)] = [the_mianzi[3]];
			switch (the_mianzi.slice(0, 3)) {
				case "123":
				case "456":
				case "789": if (yiqi_list[the_mianzi[3]].indexOf(the_mianzi.slice(0, 3)) < 0)
					yiqi_list[the_mianzi[3]].push(the_mianzi.slice(0, 3));
			}
		}
		for (let j = 0; j < fulu.length; ++j) {
			let the_mianzi = standard_mianzi(fulu[j]);
			if (sanse_list[the_mianzi.slice(0, 3)]) {
				if ((the_mianzi[3] !== "z") && (sanse_list[the_mianzi.slice(0, 3)].indexOf(the_mianzi[3]) < 0))
					sanse_list[the_mianzi.slice(0, 3)].push(the_mianzi[3]);
			}
			else
				sanse_list[the_mianzi.slice(0, 3)] = [the_mianzi[3]];
...
/**
 * 计算役满结果
 * @param {string[]} yiman_list - 役满列表
 * @param {number} beiyiman - 役满倍数
 * @param {number} fu - 符数
 * @param {number} seat - 自风(0-3)
 * @param {number} ground_wind - 场风(0-3)
 * @param {boolean} tsumo - 是否自摸
 * @returns {string} 役满结果文本
 */
/**
 * 生成役满和牌结果文本
 * @param {string[]} yiman_list - 役满列表
...
/**
 * 计算一般役种结果
 * @param {string[]} yizhong_list - 役种列表
 * @param {number} fan - 翻数
 * @param {number} fu - 符数
 * @param {number} jizhundian - 基准点
 * @param {number} dora - 宝牌数
 * @param {number} red_dora - 红宝牌数
 * @param {number} li_dora - 里宝牌数
 * @param {number} bei_dora - 北宝牌数
 * @param {number} seat - 自风
 * @param {number} ground_wind - 场风
 * @param {boolean} tsumo - 是否自摸
 * @param {boolean} menqing - 是否门清
 * @returns {string} 役种结果文本
 */
/**
 * 生成一般役种和牌结果文本
 * @param {string[]} yizhong_list - 役种列表
 ...
 * @param {number} jizhundian - 基准点
 * @param {number} fan - 翻数
 * @returns {string} 满贯大小名称（如"滿貫"、"跳滿"等）
 */
/**
 * 根据基准点和翻数判断满贯大小
 * @param {number} jizhundian - 基准点
 * @param {number} fan - 翻数
 * @returns {string} 满贯等级名称
 */
 ...
 * @param {number} jizhundian - 基准点
 * @param {number} fan - 翻数
 * @returns {number} 实际基准点数
 */
/**
 * 根据翻数计算麻将基准点数（含满贯封顶）
 * @param {number} jizhundian - 基准点
 * @param {number} fan - 翻数
 * @returns {number} 封顶后的基准点
 */
