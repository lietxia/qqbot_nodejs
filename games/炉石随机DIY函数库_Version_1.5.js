/**
 * @模块 炉石随机DIY函数库_Version_1.5.js
 * @描述 炉石传说随机随从DIY生成函数库，随机生成费用、攻击、生命、种族、职业、稀有度、描述等
 */

/**
 * 创建新的随机随从
 * @param {string} name - 随从名称
 * @returns {string} 随从完整信息文本
 */
/**
 * @模块 炉石随机DIY函数库_Version_1.5.js
 * @描述 炉石传说随机DIY随从生成函数库，包含随从创建、效果生成、种族管理等功能
 */

/**
 * 创建新的随机随从
 * @param {string} name - 随从名称
 * @returns {string} 随从卡牌文本
 */
function create_new_minion(name) {
	if (name === "")
		name = nickname + "的发现";
	if (name.slice(0, 2) !== "de" && name.length >= 25)
		return name + "\n这个名字太长啦~";
	if (name.slice(0, 2) === "de") {
		random_target = name.slice(2);
		random_place = 0;
	}
	random_record = "";
...
/**
 * 生成领主随从（基于现有卡牌3倍属性）
 * @returns {string} 领主随从完整信息文本
 */
/**
 * 生成领主随从（基于现有随从3倍属性）
 * @returns {string} 领主随从卡牌文本
 */
function new_lord_minion() {
	...
	return cost + " " + ((minion_name.length < 4) ? minion_name : minion_name.slice(0, (minion_name.length - 2))) + "领主 " + minion_class[Class] + " " + minion_rarety[rarety] + "\n" + the_description + "\n" + attack + "/" + health + (race === 0 ? "" : ("\n" + "*" + minion_race[race] + "*"));
}

/**
 * 生成随从描述（关键词+效果）
 * @param {string} name - 随从名称
 * @param {number} race - 种族索引
...
/**
 * 高山酱的战吼（发现3个暴雪设计师随从）
 * @returns {string} 3个随从信息文本
 */
/**
 * 高山酱的战吼效果（生成三个暴雪设计师随从）
 * @returns {string} 三个随从的卡牌文本
 */
function 高山酱的战吼() {
...
/**
 * 关闭指定随从种族
 * @param {string} name - 种族名称
 * @returns {string} 操作结果提示
 */
/**
 * 关闭指定随从种族
 * @param {string} name - 种族名称
 * @returns {string} 操作结果文本
 */
function 关闭种族(name) {
	if (name === "机械")
		return "机械不能被关闭哟~";
