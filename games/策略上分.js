/**
 * @模块 策略上分.js
 * @描述 三麻策略上分模拟模块，模拟双号策略和单号上分所需次数
 */

/**
 * 策略上分：只打低分号
 * @param {number[]} accounts - 两个账号的分数数组
 * @param {number} max_score - 每次最大分数变化
 * @param {number} possitivity - 正和偏移量
 * @returns {string} 上分结果
 */
 ...
 * @param {number} [possitivity=0] - 正和偏移量
 * @param {number} [max_score=100] - 每次最大分数变化
 * @returns {string} 所需次数结果
 */
...
/**
 * 解析上分分析命令
 * @param {string} order_string - 格式为"目标,正和,最大分值"
 * @returns {string} 分析结果
 */
