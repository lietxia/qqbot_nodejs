/**
 * @模块 湮灭之战改.js
 * @描述 湮灭之战游戏改进版，增加了文件持久化和四参数命令解析
 */

const fs = require('fs');

/**
 * 湮灭之战改进版命令解析入口函数(支持四参数)
...
function annihilateWar_function(order_string) {
	...
	if (order_option === "攻击" && order_name !== "" && order_detail !== "" && order_plus !== "")
		return (type === "group" ? "\n" : "") + aw攻击(order_name, order_detail, (function(){try{return JSON.parse(order_plus)}catch(e){return 0}})());
...
function annihilateWarRun() {
	...
	fs.writeFileSync('./data/annihilateWar_log.txt', log_list);
}
