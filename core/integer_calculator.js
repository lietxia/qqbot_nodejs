/**
 * @模块 integer_calculator.js
 * @描述 整数运算模块，基于查表法实现正整数和整数的加减乘除、比较、开方等运算
 */

/* ==== 正整数基础运算 ==== */

/**
 * 反转数字字符串
 * @param {string} int_a - 正整数字符串
 * @returns {string} 反转后的字符串
 */
/**
 * @模块 integer_calculator.js
 * @描述 大整数精确计算器，支持正整数和整数的加减乘除、幂运算、阶乘、精确开方等运算
 */

/**
 * 将正整数字符串反转
 * @param {string} int_a - 正整数字符串
 * @returns {string} - 反转后的字符串
 */
function positive_anti(int_a) {
anti_a = "";
for (let i = 0; i < int_a.length; ++i)
anti_a += int_a[int_a.length - i - 1];
return anti_a;
}

/**
 * 去除正整数前导零
 * @param {string} int_a - 正整数字符串
 * @returns {string} 化简后的正整数字符串
 */
/**
 * 去除正整数前导零
 * @param {string} int_a - 正整数字符串
 * @returns {string} - 去除前导零后的字符串
 */
function positive_simplify(int_a) {
let i = 0;
while (int_a[i] === "0")
++i;
if (i === int_a.length)
--i;
return int_a.slice(i);
}

/**
 * 正整数加法（查表法）
 * @param {string} a - 正整数字符串
 * @param {string} b - 正整数字符串
 * @returns {string} 和的字符串
 */
/**
 * 正整数加法
 * @param {string} a - 正整数字符串
 * @param {string} b - 正整数字符串
 * @returns {string} - 和的字符串表示
 */
function positive_add(a, b) {
if (Date.now() - calculator_start_time > Max_cal_time)
return "NaN";
if (!is_positive(a) || !is_positive(b))
return "NaN";
let anti_a = positive_anti(a);
let anti_b = positive_anti(b);
let sum = "";
let carry = 0;
for (let i = 0; i < anti_a.length || i < anti_b.length; ++i) {
sum += integer_addlist[carry][(i >= anti_a.length) ? 0 : anti_a[i]][(i >= anti_b.length) ? 0 : anti_b[i]][0];
carry = integer_addlist[carry][(i >= anti_a.length) ? 0 : anti_a[i]][(i >= anti_b.length) ? 0 : anti_b[i]][1];
}
if (carry === 1)
sum += 1;
return positive_simplify(positive_anti(sum));
}

/**
 * 正整数乘法（查表法）
 * @param {string} a - 正整数字符串
 * @param {string} b - 正整数字符串
 * @returns {string} 积的字符串
 */
/**
 * 正整数乘法
 * @param {string} a - 正整数字符串
 * @param {string} b - 正整数字符串
 * @returns {string} - 积的字符串表示
 */
function positive_multiply(a, b) {
if (Date.now() - calculator_start_time > Max_cal_time)
return "NaN";
if (!is_positive(a) || !is_positive(b))
return "NaN";
if (a === "0" || b === "0")
return "0";
if (a === "1")
return positive_simplify(b);
if (b === "1")
return positive_simplify(a);
let anti_a = positive_anti(a);
let anti_b = positive_anti(b);
let product = "0";
for (let i = 0; i < (anti_a.length) + (anti_b.length) - 1; ++i) {
let carry_product = "0";
for (let j = 0; j <= i; ++j)
carry_product = positive_add(carry_product, integer_multiplylist[(j >= anti_a.length) ? 0 : anti_a[j]][(i >= anti_b.length + j) ? 0 : anti_b[i - j]])
for (let j = 0; j < i; ++j)
carry_product += "0";
product = positive_add(product, carry_product);
}
return positive_simplify(product);
}

/**
 * 正整数幂运算（快速幂）
 * @param {string} a - 底数字符串
 * @param {string} b - 指数字符串
 * @returns {string} 幂的字符串
 */
/**
 * 正整数幂运算
 * @param {string} a - 底数（正整数字符串）
 * @param {string} b - 指数（正整数字符串）
 * @returns {string} - 幂的字符串表示
 */
function positive_power(a, b) {
if (a === "0" && b === "0")
return "NaN";
if (a === "0")
return "0";
if (b === "0")
return "1";
if (b === "1")
return a;
if (Date.now() - calculator_start_time > Max_cal_time)
return "NaN";
let half_b = positive_divide(b, "2");
let half_power = positive_power(a, half_b[0]);
let remain_power = positive_power(a, half_b[1]);
return positive_multiply(half_power, positive_multiply(half_power, remain_power));
}

/* ==== 正整数比较运算 ==== */

/**
 * 正整数大于比较
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 正整数字符串
 * @returns {boolean} int_a > int_b
 */
/**
 * 判断正整数int_a是否大于int_b
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 正整数字符串
 * @returns {boolean} - int_a > int_b
 */
function positive_greater(int_a, int_b) {
if (int_a.length != int_b.length)
return int_a.length > int_b.length;
return int_a > int_b;
}

/**
 * 正整数大于等于比较
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 正整数字符串
 * @returns {boolean} int_a >= int_b
 */
/**
 * 判断正整数int_a是否大于等于int_b
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 正整数字符串
 * @returns {boolean} - int_a >= int_b
 */
function positive_equal_or_greater(int_a, int_b) {
if (int_a.length != int_b.length)
return int_a.length > int_b.length;
return int_a >= int_b;
}

/**
 * 正整数小于比较
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 正整数字符串
 * @returns {boolean} int_a < int_b
 */
/**
 * 判断正整数int_a是否小于int_b
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 正整数字符串
 * @returns {boolean} - int_a < int_b
 */
function positive_smaller(int_a, int_b) {
if (int_a.length != int_b.length)
return int_a.length < int_b.length;
return int_a < int_b;
}

/**
 * 正整数小于等于比较
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 正整数字符串
 * @returns {boolean} int_a <= int_b
 */
/**
 * 判断正整数int_a是否小于等于int_b
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 正整数字符串
 * @returns {boolean} - int_a <= int_b
 */
function positive_equal_or_smaller(int_a, int_b) {
if (int_a.length != int_b.length)
return int_a.length < int_b.length;
return int_a <= int_b;
}

/* ==== 正整数差与除法 ==== */

/**
 * 正整数差（补数法）
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 正整数字符串
 * @returns {string} 差的字符串
 */
/**
 * 正整数差（大减小）
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 正整数字符串
 * @returns {string} - |int_a - int_b| 的字符串表示
 */
function positive_difference(int_a, int_b) {
if (Date.now() - calculator_start_time > Max_cal_time)
return "NaN";
if (!is_positive(int_a) || !is_positive(int_b))
return "NaN";
if (int_a === int_b)
return "0";
if (int_a === "0")
return int_b;
if (int_b === "0")
return int_a;
let greater_int = int_a;
let smaller_int = int_b;
if (positive_smaller(int_a, int_b)) {
greater_int = int_b;
smaller_int = int_a;
}
let smaller_complement = "";
for (let i = 0; i < greater_int.length; ++i)
smaller_complement += (i < (greater_int.length - smaller_int.length)) ? "9" : single_integer_complementary[smaller_int[i + smaller_int.length - greater_int.length]];
let unrevised_difference = positive_add(positive_add(greater_int, positive_simplify(smaller_complement)), "1");
return positive_simplify(unrevised_difference.slice(1));
}


/**
 * 正整数带余除法
 * @param {string} int_a - 被除数字符串
 * @param {string} int_b - 除数字符串
 * @returns {Array} [商, 余数] 字符串数组
 */
/**
 * 正整数除法，返回商和余数
 * @param {string} int_a - 被除数（正整数字符串）
 * @param {string} int_b - 除数（正整数字符串）
 * @returns {Array<string>} - [商, 余数]
 */
function positive_divide(int_a, int_b) {
if (!is_positive(int_a) || !is_positive(int_b))
return "NaN";
if (int_b === "0")
return "NaN";
if (positive_smaller(int_a, int_b))
return ["0", int_a];
if (int_b === "1")
return [positive_simplify(int_a), "0"];
if (int_a === int_b)
return ["1", "0"];
let deci = 1;
if (positive_equal_or_greater(int_a.slice(0, int_b.length), int_b))
while (positive_equal_or_greater(int_a.slice(0, int_b.length), positive_multiply(JSON.stringify(deci), int_b)))
++deci;
else
while (positive_equal_or_greater(int_a.slice(0, int_b.length + 1), positive_multiply(JSON.stringify(deci), int_b)))
++deci;
deci = JSON.stringify(deci - 1);
for (let i = 0; i < int_a.length - int_b.length - 1; ++i)
deci += "0";
if (positive_equal_or_greater(int_a.slice(0, int_b.length), int_b) && int_a.length > int_b.length)
deci += "0";
let reduced_division = positive_difference(int_a, positive_multiply(int_b, deci[0]) + deci.slice(1));
let next_divide = positive_divide(reduced_division, int_b);
return [positive_add(deci, next_divide[0]), next_divide[1]];
}

/* ==== 整数类型判断 ==== */

/**
 * 判断是否为正整数字符串
 * @param {string} int_a - 待判断的字符串
 * @returns {boolean} 是否为正整数
 */
/**
 * 判断字符串是否为正整数
 * @param {string} int_a - 待检测字符串
 * @returns {boolean} - 是否为正整数
 */
function is_positive(int_a) {
for (let i = 0; i < int_a.length; ++i)
if (int_a[i] < "0" || int_a[i] > "9")
return false;
return true;
}

/**
 * 判断是否为整数字符串
 * @param {string} int_a - 待判断的值
 * @returns {boolean} 是否为整数
 */
/**
 * 判断字符串是否为整数（含负数）
 * @param {string} int_a - 待检测字符串
 * @returns {boolean} - 是否为整数
 */
function is_integer(int_a) {
if (typeof (int_a) !== "string")
return false;
if (int_a[0] === "-")
return is_positive(int_a.slice(1));
return is_positive(int_a);
}

/* ==== 整数四则运算 ==== */

/**
 * 整数加法
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 整数字符串
 * @returns {string} 和的字符串
 */
/**
 * 整数加法
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 整数字符串
 * @returns {string} - 和的字符串表示
 */
function integer_add(int_a, int_b) {
if (!is_integer(int_a) || !is_integer(int_b))
return "NaN";
if (int_a[0] === "-")
if (int_b[0] === "-")
return "-" + positive_add(int_a.slice(1), int_b.slice(1));
else
if (positive_equal_or_greater(int_b, int_a.slice(1)))
return positive_difference(int_a.slice(1), int_b);
else
return "-" + positive_difference(int_a.slice(1), int_b);
else
if (int_b[0] !== "-")
return positive_add(int_a, int_b);
else
if (positive_equal_or_greater(int_a, int_b.slice(1)))
return positive_difference(int_a, int_b.slice(1));
else
return "-" + positive_difference(int_a, int_b.slice(1));
}

/**
 * 整数减法
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 整数字符串
 * @returns {string} 差的字符串
 */
/**
 * 整数减法
 * @param {string} int_a - 被减数（整数字符串）
 * @param {string} int_b - 减数（整数字符串）
 * @returns {string} - 差的字符串表示
 */
function integer_minus(int_a, int_b) {
if (int_b[0] === "-")
return integer_add(int_a, int_b.slice(1));
else
return integer_add(int_a, "-" + int_b);
}

/**
 * 整数乘法
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 整数字符串
 * @returns {string} 积的字符串
 */
/**
 * 整数乘法
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 整数字符串
 * @returns {string} - 积的字符串表示
 */
function integer_multiply(int_a, int_b) {
if (!is_integer(int_a) || !is_integer(int_b))
return "NaN";
if (int_a[0] === "-")
if (int_b[0] === "-")
return positive_multiply(int_a.slice(1), int_b.slice(1));
else
return "-" + positive_multiply(int_a.slice(1), int_b);
else
if (int_b[0] !== "-")
return positive_multiply(int_a, int_b);
else
return "-" + positive_multiply(int_a, int_b.slice(1));
}

/**
 * 整数带余除法
 * @param {string} int_a - 被除数字符串
 * @param {string} int_b - 除数字符串
 * @returns {Array} [商, 余数] 字符串数组
 */
/**
 * 整数除法，返回商和余数
 * @param {string} int_a - 被除数（整数字符串）
 * @param {string} int_b - 除数（整数字符串）
 * @returns {Array<string>} - [商, 余数]
 */
function integer_divide(int_a, int_b) {
if (!is_integer(int_a) || !is_integer(int_b))
return ["NaN", "0"];
if (int_a[0] === "-")
if (int_b[0] === "-") {
let absolute_divide = positive_divide(int_a.slice(1), int_b);
if (absolute_divide[1] !== "0")
return [positive_add(absolute_divide[0], "1"), positive_difference(absolute_divide[1], int_b)];
else
return [absolute_divide[0], "0"];
}
else {
let absolute_divide = positive_divide(int_a.slice(1), int_b);
if (absolute_divide[1] !== "0")
return ["-" + positive_add(absolute_divide[0], "1"), positive_difference(absolute_divide[1], int_b)];
else
return ["-" + absolute_divide[0], "0"];
}
else
if (int_b[0] !== "-")
return positive_divide(int_a, int_b);
else {
let absolute_divide = positive_divide(int_a, int_b.slice(1));
return ["-" + absolute_divide[0], absolute_divide[1]];
}
}

/* ==== 整数辅助运算 ==== */

/**
 * 整数取负
 * @param {string} int_a - 整数字符串
 * @returns {string} 取负后的整数字符串
 */
/**
 * 整数取负
 * @param {string} int_a - 整数字符串
 * @returns {string} - 相反数的字符串表示
 */
function neg_integer(int_a) {
if (!is_integer(int_a))
return "NaN";
if (int_a[0] === "-")
return int_a.slice(1);
if (int_a === "0")
return "0";
return "-" + int_a;
}

/**
 * 整数化简，去除前导零
 * @param {string} int_a - 整数字符串
 * @returns {string} 化简后的整数字符串
 */
/**
 * 整数化简（去除前导零）
 * @param {string} int_a - 整数字符串
 * @returns {string} - 化简后的字符串
 */
function integer_simplify(int_a) {
if (!is_integer(int_a))
return "NaN";
let absolute = "";
if (int_a[0] === "-")
absolute = positive_simplify(int_a.slice(1));
else
absolute = positive_simplify(int_a);
if (absolute === "0")
return "0";
return ((int_a[0] === "-") ? "-" : "") + absolute;
}

/**
 * 整数阶乘
 * @param {string} int_a - 非负整数字符串
 * @returns {string} 阶乘结果字符串
 */
/**
 * 正整数阶乘
 * @param {string} int_a - 正整数字符串
 * @returns {string} - 阶乘结果的字符串表示
 */
function integer_factorial(int_a) {
if (!is_positive(int_a))
return "NaN";
if (positive_greater(int_a, "251"))
return "NaN";
if (int_a === "0")
return "1";
return positive_multiply(int_a, integer_factorial(positive_difference(int_a, "1")));
}

/**
 * 生成10的n次方字符串
 * @param {number} n - 指数
 * @returns {string} 10^n 的字符串
 */
/**
 * 生成10的n次幂的字符串
 * @param {number} n - 幂次
 * @returns {string} - 10^n 的字符串表示
 */
function power_10(n) {
let number = "1";
for (let i = 0; i < n; ++i)
number += "0";
return number;
}

/* ==== 整数比较运算 ==== */

/**
 * 整数大于比较
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 整数字符串
 * @returns {boolean} int_a > int_b
 */
/**
 * 判断整数int_a是否大于int_b
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 整数字符串
 * @returns {boolean} - int_a > int_b
 */
function integer_greater(int_a, int_b) {
if (!is_integer(int_a) || !is_integer(int_b))
return false;
if (int_a[0] === "-")
if (int_b[0] === "-")
return positive_smaller(int_a.slice(1), int_b.slice(1));
else
return false;
else
if (int_b[0] === "-")
return true;
else
return positive_greater(int_a, int_b);
}

/**
 * 整数大于等于比较
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 整数字符串
 * @returns {boolean} int_a >= int_b
 */
/**
 * 判断整数int_a是否大于等于int_b
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 整数字符串
 * @returns {boolean} - int_a >= int_b
 */
function integer_equal_or_greater(int_a, int_b) {
if (!is_integer(int_a) || !is_integer(int_b))
return false;
if (int_a[0] === "-")
if (int_b[0] === "-")
return positive_equal_or_smaller(int_a.slice(1), int_b.slice(1));
else
return false;
else
if (int_b[0] === "-")
return true;
else
return positive_equal_or_greater(int_a, int_b);
}

/**
 * 整数小于比较
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 整数字符串
 * @returns {boolean} int_a < int_b
 */
/**
 * 判断整数int_a是否小于int_b
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 整数字符串
 * @returns {boolean} - int_a < int_b
 */
function integer_smaller(int_a, int_b) {
if (!is_integer(int_a) || !is_integer(int_b))
return false;
if (int_a[0] === "-")
if (int_b[0] === "-")
return positive_greater(int_a.slice(1), int_b.slice(1));
else
return false;
else
if (int_b[0] === "-")
return true;
else
return positive_smaller(int_a, int_b);
}

/**
 * 整数小于等于比较
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 整数字符串
 * @returns {boolean} int_a <= int_b
 */
/**
 * 判断整数int_a是否小于等于int_b
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 整数字符串
 * @returns {boolean} - int_a <= int_b
 */
function integer_equal_or_smaller(int_a, int_b) {
if (!is_integer(int_a) || !is_integer(int_b))
return false;
if (int_a[0] === "-")
if (int_b[0] === "-")
return positive_equal_or_greater(int_a.slice(1), int_b.slice(1));
else
return false;
else
if (int_b[0] === "-")
return true;
else
return positive_equal_or_smaller(int_a, int_b);
}

/* ==== 整数开方与幂分解 ==== */

/**
 * 精确开整数次方根
 * @param {string} int_a - 被开方整数字符串
 * @param {string} int_b - 开方次数字符串
 * @returns {string} 精确根字符串，若非完全幂则返回"NaN"
 */
/**
 * 精确开b次方（二分搜索）
 * @param {string} int_a - 被开方数（整数字符串）
 * @param {string} int_b - 开方次数（整数字符串）
 * @returns {string} - 精确整数根或"NaN"
 */
function exact_root(int_a, int_b) {
if (Date.now() - calculator_start_time > Max_cal_time)
return "NaN";
if (int_a[0] === "-") {
if (positive_divide(int_b[int_b.length - 1], "2")[1] === "0")
return "NaN";
else {
let neg_root = exact_root(int_a.slice(1), int_b);
return (neg_root === "NaN") ? "NaN" : ("-" + neg_root);
}
}
let itt_up = power_10(Math.ceil(int_a.length / (function(){try{return JSON.parse(int_b)}catch(e){return NaN}})()));
let itt_down = positive_difference(power_10(Math.ceil(int_a.length / (function(){try{return JSON.parse(int_b)}catch(e){return NaN}})()) - 1), "1");
let itt_test = positive_divide(positive_add(itt_up, itt_down), "2")[0];
while (itt_test != itt_up && itt_test != itt_down) {
let itt_power = positive_power(itt_test, int_b);
if (itt_power === int_a)
return itt_test;
if (positive_greater(itt_power, int_a))
itt_up = itt_test;
else
itt_down = itt_test;
itt_test = positive_divide(positive_add(itt_up, itt_down), "2")[0];
}
return "NaN";
}

/**
 * 整数幂分解，提取最大b次方因子
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 次数字符串
 * @returns {Array} [因子, 剩余部分] 字符串数组
 */
/**
 * 整数幂分解，返回最大t使得t^b整除int_a
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 幂次（整数字符串）
 * @returns {Array<string>} - [t, int_a/t]
 */
function integer_power_decompose(int_a, int_b) {
if (int_a[0] === "-") {
let p_divisor = integer_power_decompose(int_a.slice(1), int_b);
return [p_divisor[0], "-" + p_divisor[1]];
}
if (int_a === "0")
return ["0", "1"];
if (int_a === "1")
return ["1", "1"];
return max_p_divisor(int_a, int_b);
}

/* 给定整数a,b>1，返回最大的t开b次方(和a除以t)，使得t整除a且t开b次方是整数。 */

/**
 * 求最大b次方因子
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 次数字符串
 * @param {string} prime_start - 起始素数，默认"2"
 * @returns {Array} [因子, 剩余部分] 字符串数组
 */
/**
 * 递归寻找最大t使得t^b整除int_a
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 幂次（正整数字符串）
 * @param {string} [prime_start="2"] - 起始素数
 * @returns {Array<string>} - [t, int_a/t]
 */
function max_p_divisor(int_a, int_b, prime_start = "2") {
for (let i = prime_start; positive_smaller(i, "400"); i = positive_add(i, "1"))
if (prime_list[i]) {
let try_divisor = positive_power(i, int_b);
if (positive_greater(try_divisor, int_a))
return ["1", int_a];
let check_divisor = positive_divide(int_a, try_divisor);
if (check_divisor[1] === "0") {
let next_result = max_p_divisor(check_divisor[0], int_b, i);
return [positive_multiply(i, next_result[0]), next_result[1]];
}
}
return ["1", int_a];
}

/**
 * 整数幂次检查，求最大指数t<b使得a开t次方为整数
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 上界字符串
 * @returns {Array} [根, 指数] 字符串数组
 */
/**
 * 整数幂检测，返回最大t<b使得int_a开t次方为整数
 * @param {string} int_a - 整数字符串
 * @param {string} int_b - 上界（整数字符串）
 * @returns {Array<string>} - [根, 指数t]
 */
function integer_check_power(int_a, int_b) {
if (int_a[0] === "-") {
let p_power = integer_check_power(int_a.slice(1), int_b);
while (positive_divide(p_power[1], "2")[1] === "0")
p_power = [positive_multiply(p_power[0], p_power[0]), positive_divide(p_power[1], "2")[0]];
return ["-" + p_power[0], p_power[1]];
}
if (int_a === "0")
return ["0", positive_difference(int_b, "1")];
if (int_a === "1")
return ["1", positive_difference(int_b, "1")];
return max_p_exponent(int_a, int_b);
}

/* 给定整数a,b>1，返回最大整数t<b(和a开t次方)，使得a开t次方是整数。 */

/**
 * 求最大幂指数使a开方为整数
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 上界字符串
 * @returns {Array} [根, 指数] 字符串数组
 */
/**
 * 递归寻找最大指数t<b使得int_a开t次方为整数
 * @param {string} int_a - 正整数字符串
 * @param {string} int_b - 上界（正整数字符串）
 * @returns {Array<string>} - [根, 指数t]
 */
function max_p_exponent(int_a, int_b) {
if (positive_greater(int_b, "103"))
return ["NaN", "0"];
let mod = positive_divide(int_a, int_b)[1];
if (mod === "0" || !prime_list[int_b]) {
let guess_result = max_p_exponent(int_a, positive_add(int_b, "1"));
if (positive_greater(guess_result[1], int_b))
return [positive_power(guess_result[0], prime_decompose[guess_result[1]][0]), positive_divide(guess_result[1], prime_decompose[guess_result[1]][0])];
return guess_result;
}
let check_mod = mod;
let i = 1;
for (; check_mod !== "1"; ++i)
check_mod = positive_divide(positive_multiply(check_mod, mod), int_b)[1];
let k = JSON.stringify(i);
let order = positive_difference(int_b, "1");
for (let t = order; t !== "0"; t = positive_difference(t, "1"))
if (positive_divide(order, positive_multiply(k, gcd(order, t)))[1] !== "0")
continue;
else {
let guess_root = exact_root(int_a, t);
if (guess_root !== "NaN")
return [guess_root, t];
}
return ["NaN", "0"];
}